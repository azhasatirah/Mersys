...
@section('title','Master batas ganti jadwal')
@section('content')
...
@push('scripts')
    <script>
        ...
        function getData(){
            $.get(UrlLevel+"master/maksimalchangejadwal/getdata" ).done(ele=>{
                master_data = ele
                showData()  
            })
        }
        ...
        function editData(id){
            ...
            $('#medit-data').val(data[0].jam)
            $('#medit-idmaster').val(data[0].id)
        }
        function update(){
            $.ajax({
                type: "post",
                url: UrlLevel+"master/maksimalchangejadwal/update",
